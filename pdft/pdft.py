...
class U_Embedding:
    ...
    def fragments_scf_array_basis(self, max_iter, vp=None, vp_fock=None, printflag=False):
        """
        Run scf calculations for each fragment with given vp or vp_fock,
    ...
    def fragments_scf_matrix_basis(self, max_iter, vp=None, vp_fock=None, printflag=False):
        """
        Run scf calculations for each fragment with given vp or vp_fock,
        vp is now on 1 basis: vp = sum b_ij phi_i phi_j. In this case, 4-overlap needed.
        """
        # Run the whole molecule SCF calculation if not calculated before.
        if self.molecule.Da is None:
            self.molecule.scf(maxiter=max_iter, print_energies=printflag)
